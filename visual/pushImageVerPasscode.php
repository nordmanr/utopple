...
<html>
	...
	<table>
		<?php
			...
			$actualPasscode = file_get_contents("./pythonCreated/$passcode.passcode");		// Server previously posted passcode

			if($passcode == $actualPasscode){					// Verified from current user
				$file = fopen($size."x".$size.".csv", "w") or die("Unable to open!");
				fwrite($file, $updatedColors);
				fclose($file);
				echo "VALID PASSCODE - POSTED NEW COLORS";
			}else{
